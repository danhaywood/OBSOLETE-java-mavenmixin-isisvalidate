...
echo ""
mvn clean package -Dskip.danhaywood-mavenmixin-isisvalidate -o >/dev/null
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
...
echo ""
mvn clean deploy -Pdanhaywood-mavenmixin-sonatyperelease -Dskip.danhaywood-mavenmixin-isisvalidate -Dpgp.secretkey=keyring:id=$KEYID -Dpgp.passphrase="literal:$PASSPHRASE"
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
